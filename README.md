...
> 1. если `RUN_DB_SETUP=true` → выполняется `bin/deploy/delete-create-db.mjs`, который
>    **УДАЛЯЕТ и пересоздаёт БД**. В проде после первичной инициализации ставить **`RUN_DB_SETUP=false`**.
